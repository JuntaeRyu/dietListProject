...
.conditionCheck ul li {
	width: calc(40%); /* 추가: 각 li 요소의 너비 조절 */
	margin-bottom: 5px; /* 추가: 각 li 요소의 하단 마진 조절 */
}
...
<body>
					...
					<div style="display: flex; align-items: center;">
						...
						<ul>
							...
							<li>식당명 <select id="restaurantName" name="restaurantName">
									<c:forEach var="rName" items="${restaurantNameSearch}">
										<option value="${rName.value}">${rName.key}</option>
									</c:forEach>
							</select>
							</li>
							<li>식사 구분 <select id="mealTime" name="mealTime">
									<c:forEach var="mealTime" items="${mealTimeSearch}">
										<option value="${mealTime.value}">${mealTime.key}</option>
									</c:forEach>
							</select>
							</li>
							<li>목록 갯수 <select id="listCount" name="listCount">
									<c:forEach var="listCount" items="${listCountSearch}">
										<option value="${listCount.value}">${listCount.key}</option>
									</c:forEach>
							</select></li>
						</ul>
							<input type="submit" value="검색" style="margin-right: 30px; width: 100px; height: 60px;">
						</form>
					</div>
				</div>
	...
	<script>
		function updateEndDateMinMax() {
			...
			// 만약 시작 날짜가 일요일이면 최대 날짜를 시작 날짜로 설정
			if (startDate.getDay() === 0) {
				endDate = startDate;
			}

			// "YYYY-MM-DD" 형식으로 종료 날짜 포맷
